...
- **PSA (v1 semantics)**: global params draw once per iteration and are shared everywhere. A sub-model's own dist-bearing params draw per attachment (two attachments of the same sub-model sample independently, reproducibly). To share uncertainty across attachments, route the parameter through a global param via `with:` (e.g. `with: {p_prog: p_prog_global}`). Cross-attachment pre-drawing is a planned phase-2 refinement.
