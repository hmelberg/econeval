...
/* Selection and hover must not look alike, or a merely-hovered row reads as the selected one.
   Selection keeps the accent tint AND gains an accent edge; hover is a plainer surface tint. */
.otl-row[aria-current="true"] {
  background: var(--accent-soft);
  box-shadow: inset 2px 0 0 var(--accent);
}
.otl-row:hover { background: var(--line); }
.otl-row[aria-current="true"]:hover { background: var(--accent-soft); }
