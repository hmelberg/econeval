...
def update_history(self):
    ...
    if self.new_state == self.state:
        ...
        self.model.history[self.unique_id].append((self.new_state, 
                                                  self.attr['age'], 
...
class SimAgent(Agent):
    ...
    def __init__(self, unique_id, model):
        ...
        self.model.history[self.unique_id] = [(self.attr['age'], self.state)]
...
def add_state_value(self, attr, **kwargs):
    a = self.attr[attr] + self.model.state_attr[self.state][state_attr]
    return a
...
states = [('susceptible',           {'u':0.6, 'c':100}),
